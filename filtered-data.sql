-- Filtered views for root_group_id = {{ filtered_root_group_id }}
-- Generated from template. Do not edit directly.

-- ============================================================================
-- GROUPS (recursive hierarchy from root group)
-- This is the foundation for all other filtered views
-- ============================================================================

CREATE OR REPLACE VIEW `{{ target_dataset_name }}.groups` AS
WITH RECURSIVE group_tree AS (
  -- Root: the starting group itself
  SELECT
  ...
  WHERE SAFE_CAST(id AS INT64) = {{ filtered_root_group_id }}

  UNION ALL

  -- Children, grandchildren, etc.
  SELECT
    SAFE_CAST(g.id AS INT64)        AS id,
    SAFE_CAST(g.parent_id AS INT64) AS parent_id,
    g.*,
    gt.depth + 1                    AS depth
  FROM `actionnetwork_cleaned.cln_actionnetwork__groups` g
  JOIN group_tree gt
    ON SAFE_CAST(g.parent_id AS INT64) = gt.id
  WHERE gt.depth < 10   -- safety limit
)
SELECT * EXCEPT(depth) FROM group_tree;

-- ============================================================================
-- FILTERING BY GROUP ID
-- Tables that have group_id, filtered via the groups view
-- ============================================================================
...
WHERE group_id IN (SELECT id FROM `{{ target_dataset_name }}.groups`);
...
-- Users table filtered via root_group_id -> groups view
-- ============================================================================

CREATE OR REPLACE VIEW `{{ target_dataset_name }}.users` AS
SELECT *
FROM `actionnetwork_cleaned.cln_actionnetwork__users`
WHERE root_group_id IN (SELECT id FROM `{{ target_dataset_name }}.groups`);

-- ============================================================================
-- FILTERING BY FUNDRAISING_ID
-- Tables filtered via fundraising_id -> fundraisings.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via event_id -> events.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via petition_id -> petitions.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via campaign_id -> campaigns.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via call_campaign_id -> call_campaigns.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via letter_id -> letters.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via email_id -> emails.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via event_campaign_id -> event_campaigns.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via mobile_message_id -> mobile_messages.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via ticketed_event_id -> ticketed_events.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via ladder_id -> ladders.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via form_id -> forms.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via page_wrapper_id -> page_wrappers.group_id -> groups view
-- ============================================================================
...
-- Tables filtered via core_field_id -> core_fields.owner_id -> groups view
-- ============================================================================
...
-- Tables filtered via field_name_id -> field_names.owner_id -> groups view
-- ============================================================================
